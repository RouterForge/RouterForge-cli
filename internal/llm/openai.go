...
func (p *OpenAIProvider) Chat(ctx context.Context, req ChatRequest) (*ChatResponse, error) {
	...
	if err != nil {
		return nil, &ProviderError{Message: "openai api call", Cause: err}
	}
	...
	if len(openAIResp.Choices) == 0 {
		return nil, &ProviderError{Message: "no choices"}
	}
...
func (p *OpenAIProvider) ChatStream(ctx context.Context, req ChatRequest) (<-chan StreamEvent, error) {
	...
	resp, err := p.client.Do(httpReq)
	if err != nil {
		return nil, &ProviderError{Message: "stream call", Cause: err}
	}
